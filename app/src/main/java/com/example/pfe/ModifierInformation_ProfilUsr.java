...
public class ModifierInformation_ProfilUsr extends AppCompatActivity {
    Toolbar Toolbar_Modifier_ProfilUsr;
    String IDUsr_ProfilUsr,Adresse_Courriel_ProfilUsr,Nom_ProfilUsr,Prenom_ProfilUsr,DateDeNaissance_ProfilUsr,Sexe_ProfilUsr,NumTel_ProfilUsr,MDP_ProfilUsr;
    EditText Adresse_Courriel_MdfUsr,Nom_MdfUsr,Prenom_MdfUsr,NumTel_MdfUsr;
    Spinner DDN_jour_MdfUsr,DDN_Mois_MdfUsr,DDN_Annee_MdfUsr,Sexe_MdfUsr;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_modifier_information__profil_usr);
        ...
        IDUsr_ProfilUsr = intent.getStringExtra(Menu_Usr.IDUsr_MdfUsr);
        Adresse_Courriel_ProfilUsr = intent.getStringExtra(Menu_Usr.Adresse_Courriel_MdfUsr);
        Nom_ProfilUsr = intent.getStringExtra(Menu_Usr.Nom_MdfUsr);
        Prenom_ProfilUsr = intent.getStringExtra(Menu_Usr.Prenom_MdfUsr);
        DateDeNaissance_ProfilUsr = intent.getStringExtra(Menu_Usr.DateDeNaissannce_MdfUsr);
        Sexe_ProfilUsr = intent.getStringExtra(Menu_Usr.Sexe_MdfUsr);
        NumTel_ProfilUsr = intent.getStringExtra(Menu_Usr.NumTel_MdfUsr);
        MDP_ProfilUsr = intent.getStringExtra(Menu_Usr.MDP_MdfUsr);
                ...
                if (!Window_Modifier_Mdp.getText().toString().trim().equals("") && !Window_Modifier_Mdp_New.getText().toString().trim().equals("") && !Window_Modifier_Mdp_Conf_New.getText().toString().trim().equals("")){
                    ...
                    if (MDP_Actuel.equals(MDP_ProfilUsr)){
                        if (Window_Modifier_Mdp_New.getText().toString().equals(Window_Modifier_Mdp_Conf_New.getText().toString())){
                            ...
                            Toast.makeText(ModifierInformation_ProfilUsr.this, "Nouveau mot de passe est pas identique", Toast.LENGTH_SHORT).show();
                        }
                    }else{
                        Toast.makeText(ModifierInformation_ProfilUsr.this, "Mot de passe erroné", Toast.LENGTH_LONG).show();
                    }
                }else{
                    Toast.makeText(ModifierInformation_ProfilUsr.this, "Veuillez remplire tous les champs.", Toast.LENGTH_LONG).show();
                }
    ...
    public void Modifier_Information(View view){
        if (!Adresse_Courriel_MdfUsr.getText().toString().equals(Adresse_Courriel_ProfilUsr) || !Nom_MdfUsr.getText().toString().equals(Nom_ProfilUsr) || !Prenom_MdfUsr.getText().toString().equals(Prenom_ProfilUsr) || !NumTel_MdfUsr.getText().toString().equals(NumTel_ProfilUsr) || !Sexe_MdfUsr.getSelectedItem().toString().equals("- -") || !DDN_jour_MdfUsr.getSelectedItem().toString().equals("- -") || !DDN_Mois_MdfUsr.getSelectedItem().toString().equals("- -") || !DDN_Annee_MdfUsr.getSelectedItem().toString().equals("- -")){
            ...
            ArrierePlanAsyncTask = new ArrierePlan();
            ArrierePlanAsyncTask.execute("Modifier_Information",IDUsr_ProfilUsr,Adresse_Courriel_ProfilUsr,Nom_ProfilUsr,Prenom_ProfilUsr,DateDeNaissance_ProfilUsr,Sexe_ProfilUsr,NumTel_ProfilUsr);
            ...
            switch (Operation){
                case "Modifier_Information" :
                    try {
                        ...
                        String AdrMailUsr = strings[2];
                        String NomUsr = strings[3];
                        String PrenomUsr = strings[4];
                        String DateDeNaissanceUsr = strings[5];
                        String SexeUsr = strings[6];
                        String NumTelUsr = strings[7];
                        String Url_ModifierInfo = "http://192.168.1.9:8080/ModifierInformation_ProfilUsr.php";
                        ...
                        String Post_Date = URLEncoder.encode("IDUsr", "UTF-8") + "=" + URLEncoder.encode(IDUsr, "UTF-8")+"&&"+
                                URLEncoder.encode("AdrMail", "UTF-8") + "=" + URLEncoder.encode(AdrMailUsr, "UTF-8")+"&&"+
                                URLEncoder.encode("NomUsr", "UTF-8") + "=" + URLEncoder.encode(NomUsr, "UTF-8")+"&&"+
                                URLEncoder.encode("PrenomUsr", "UTF-8") + "=" + URLEncoder.encode(PrenomUsr, "UTF-8")+"&&"+
                                URLEncoder.encode("DateDeNaissanceUsr", "UTF-8") + "=" + URLEncoder.encode(DateDeNaissanceUsr, "UTF-8")+"&&"+
                                URLEncoder.encode("SexeUsr", "UTF-8") + "=" + URLEncoder.encode(SexeUsr, "UTF-8")+"&&"+
                                URLEncoder.encode("NumTelUsr", "UTF-8") + "=" + URLEncoder.encode(NumTelUsr, "UTF-8");
            ...
            try {
                ...
                switch (resultat){
                    case "Information Profil Modifier" :
                        Toast.makeText(ModifierInformation_ProfilUsr.this, "Informations Modifiées", Toast.LENGTH_SHORT).show();
                        onBackPressed();
                        break;

                    case "MDP Modifier" :
                        Toast.makeText(ModifierInformation_ProfilUsr.this, "Mot de passe modifié", Toast.LENGTH_SHORT).show();
                        Dialog_Modifier_Mdp.dismiss();
                        break;
                }
